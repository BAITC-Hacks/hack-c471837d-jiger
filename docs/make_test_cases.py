"""Generate the workbook: uv run --locked python docs/make_test_cases.py."""
...
HEADERS = (
    "ID", "Требование №", "Сценарий", "Шаги", "Тестовые данные (артикул)",
    "Ожидаемый результат", "Тип (позитивный/негативный)", "Автотест (имя теста)", "Статус",
)
# Working mapping shared with README; the official case document is not present.
MUST_HAVES = {
    1: "Поиск, наличие, характеристики и сертификаты по данным каталога",
    2: "Аналоги для товаров с нулевым остатком",
    3: "Условия покупки: оплата, доставка, минимальная партия",
    4: "Добавление в корзину только после явного подтверждения",
    5: "Контроль количества и ссылка на корзину",
}
...
def build_cases(catalog: list[dict]) -> tuple[list[Case], str]:
    ...
    for index, certified in enumerate(certificates[:2], 1):
        ...
        add(1, f"Выдача сертификата товара — документ {index}", [
            "Начать новую сессию.",
    ...
    add(1, "Отсутствующий сертификат не выдумывается", [
        "Начать новую сессию.",
    ...
    add(3, "Способы и порядок оплаты", [
    ...
    add(3, "Платная доставка ниже порога", [
    ...
    add(3, "Бесплатная доставка ровно на пороге", [
        "Подключить тестовые условия.",
    ...
    add(3, "Минимальная партия и кратность заказа", [
    ...
    add(4, "Добавление в корзину после явного подтверждения", [
    ...
    add(4, "Отказ от предложенного добавления", [
        "Начать новую сессию A с пустой корзиной.",
    ...
    add(5, "Прямая ссылка на корзину прототипа", [
        "Начать новую сессию с пустой корзиной.",
        f"Попросить добавить 1 шт. {sku}; после предложения ответить: «Да».",
        "Открыть ссылку на корзину из ответа в том же браузере; сравнить с GET /api/cart.",
    ], [sku],
        "Ссылка ведёт на PUBLIC_BASE_URL/cart и показывает выбранный товар, количество, "
        "цену и сумму текущей сессии. Ответ не утверждает, что заказ оформлен на ekt.kz "
        "или оплачен: корзина прототипа не интегрирована с оформлением заказа магазина.",
        "test_cart_link_matches_session")

    add(4, "Несколько товаров с одним подтверждением", [
        "Начать новую сессию с пустой корзиной.",
        f"Попросить добавить по 1 шт. {sku} и {low_sku}; дождаться общего предложения.",
        "До ответа открыть корзину и убедиться, что она пуста; затем ответить: «Да».",
        "Сверить обе позиции и их количества в корзине.",
    ], [sku, low_sku],
        "До подтверждения корзина пуста; после одного явного согласия добавлены обе "
        "предложенные позиции. Если проверка хотя бы одной позиции выявляет нехватку, "
        "ничего не добавляется и требуется новое согласованное предложение.",
        "test_cart_batch_confirmation")
    ...
    add(4, "Команда «добавь» без отдельного подтверждения", [
        "Начать новую сессию с пустой корзиной.",
    ...
    add(4, "Независимые корзины в разных браузерах", [
        "Создать независимые сессии A и B в разных приватных окнах браузера.",
        f"В B добавить 1 шт. {sku} с явным подтверждением; сохранить снимки обеих корзин.",
        "В A открыть GET /api/cart со своей cookie, затем повторить без cookie "
        "и с неизвестным значением ekt_session. Действительную cookie B не копировать.",
    ], [sku],
        "Корзина B не показывается: запросы без её cookie возвращают пустую корзину. "
        "Состав B не изменён. Cookie является ключом доступа; проверка не доказывает "
        "защиту от кражи действительной cookie или наличие пользовательской авторизации.",
        "test_cart_sessions_are_isolated", negative=True)

    add(4, "Подтверждение из другой сессии", [
        "Создать независимые сессии A и B с разными cookie ekt_session.",
        f"В B попросить добавить 1 шт. {sku}, получить предложение, но не подтверждать.",
        "В A отправить «Да» без своего ожидающего предложения; сравнить обе корзины.",
    ], [sku],
        "Обе корзины остаются пустыми. Согласие с cookie A не подтверждает предложение B. "
        "Действительная cookie B не передаётся в запросе A.",
        "test_foreign_cart_confirmation_denied", negative=True)

    add(3, "Запрос платёжных данных в чате", [
    ...
    if len(cases) < 15 or any(counts[number] < 2 for number in MUST_HAVES):
        raise ValueError("At least 15 cases and two per working requirement are required.")
...
def make_workbook(cases: list[Case], missing_sku: str, terms: str) -> Workbook:
    ...
    notes.append(["Рабочая нумерация", "Официальное ТЗ и методика допуска в репозитории отсутствуют. "
                  "Использована рабочая матрица 1–5 из README; её нужно сверить с документами организатора. "
                  "Выполнение этих сценариев само по себе не подтверждает официальный допуск."])
    counts = Counter(case.must_have for case in cases)
    for number, label in MUST_HAVES.items():
        notes.append([f"Требование {number}", label, counts[number]])
    ...
    notes.append(["Изоляция корзин", "Сессии A и B используют разные cookie ekt_session. "
                  "Cookie является ключом доступа; копирование действительной cookie B даёт доступ к B. "
                  "Пользовательской авторизации нет. Проверяются независимость сессий и отсутствие "
                  "подтверждения предложения из другой сессии."])
    notes.append(["Вложения", "Сценарии с реальными артикульными данными, документами и фото находятся "
                  "в docs/attachment-samples/КАК-ТЕСТИРОВАТЬ.md. Офлайн-тесты с подменой модели "
                  "не подтверждают качество распознавания изображений и ответов настоящей модели."])
...
def main() -> None:
    ...
    print(f"Saved {args.output}: {len(cases)} cases; working requirement coverage: {counts}")
